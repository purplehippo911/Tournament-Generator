...
const PlayerInput = () => {
    ...
    return (
        <div className="w-full flex flex-col gap-2 items-center" onSubmit={(e) => handleAddPlayer(e)}>
            <label htmlFor="name" id='input-label'>Participants/Team Name</label>
            ...
            <div className="w-full md:w-4/3 max-h-[90px] outline outline-2 bg-light-white py-5 px-2 grid grid-cols-3 md:grid-cols-5 gap-1.5 overflow-scroll">
                   {
                     players.map((player, key) => {
                        return (
                            <button key={`${key}`} className="flex flex-col justify-center w-full mb-6">
